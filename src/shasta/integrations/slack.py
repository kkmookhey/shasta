"""Slack integration for Shasta compliance alerts.
...
class SlackClient:
    ...
    def send_scan_summary(self, scan: ScanResult, score: ComplianceScore) -> bool:
        ...
        critical_high = [
            ...
            if f.status.value == "fail" and f.severity.value in ("critical", "high")
        ]
        if critical_high:
            finding_text = "\n".join(
        ...
        blocks.append(
            {
                ...
                "elements": [
                    {
                        "type": "mrkdwn",
                        "text": f"Scanned domains: {', '.join(d.value for d in scan.domains_scanned)}",
                    }
                ],
            }
        )
                    ...
                    "blocks": [
                        {
                            ...
                            "text": {
                                "type": "plain_text",
                                "text": f"Compliance Alert: {finding.title}",
                            },
                        },
                        {
                            "type": "section",
            ...
            payload["attachments"][0]["blocks"].append(
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": f":wrench: *Fix:* {finding.remediation[:1000]}",
                    },
                }
            )
    ...
    def send_drift_alert(
        self, new_findings: int, resolved: int, score_delta: float, trend: str
    ) -> bool:
        ...
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": "Shasta Compliance Drift Report"},
            },
            {
                "type": "section",
                "fields": [
            ...
            blocks.append(
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": ":warning: New compliance issues detected. Run `/scan` for details.",
                    },
                }
            )
