...
### 1. Multi-Cloud Security Scanning (5 Domains, 129+ Checks)
...
|----------|----------|--------|
| Technical cloud controls | ~90% | 129+ automated checks across AWS and Azure (full CIS AWS v3.0 + CIS Azure v3.0 coverage) |
...
| Remediation guidance | ~90% | 73 Terraform templates (42 AWS + 31 Azure azurerm) covering CloudTrail/KMS/Object Lock, Security Hub, Access Analyzer, EFS/SNS/SQS/Secrets/ACM encryption, ELB v2 TLS+access logs+headers, RDS deep+IAM auth+PITR, Lambda runtime+CMK+DLQ, API Gateway WAF+logging, AWS Backup vault lock, VPC endpoints, CloudWatch Logs KMS+retention, AWS Org SCPs+tag policies — plus the full Azure Storage/Key Vault/SQL/Cosmos/PostgreSQL/MySQL/App Service/Recovery Services Vault/Defender/Activity Log Alerts/governance set |
