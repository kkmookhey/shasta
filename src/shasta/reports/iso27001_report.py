...
def save_iso27001_markdown_report(scan: ScanResult, output_path: Path | str = "data/reports") -> Path:
    ...
    status_icon = {"pass": "PASS", "fail": "FAIL", "partial": "PARTIAL", "requires_policy": "POLICY", "not_assessed": "N/A"}

    lines = [
        "# ISO 27001:2022 Gap Analysis Report",
        "",
        f"**Generated:** {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}",
        f"**AWS Account:** {scan.account_id}",
    ...
    policy_controls = {k: v for k, v in controls.items() if v["overall_status"] == "requires_policy"}
    if policy_controls:
