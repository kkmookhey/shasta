...
"""
    ...
    filepath.write_text(
        _wrap(f"HIPAA Gap Analysis — {scan.account_id}", body), encoding="utf-8"
    )
    return filepath
...
def _build_findings_section(findings: list[Finding]) -> str:
    ...
    if not_assessed:
        ...
        parts.append("<p>These checks could not run due to missing permissions. "
                      "Assign <strong>Security Reader</strong> or <strong>Global Reader</strong> "
                      "in Entra ID to unlock them.</p>")
...
def save_consolidated_html_report(
    ...
    soc2_ctrls = get_control_summary(scan.findings)
    iso_ctrls = get_iso27001_control_summary(scan.findings)
